...
export const useSecureAdmin = () => {
  ...
  const verifyAdminStatus = async (): Promise<boolean> => {
    ...
    try {
      console.log('🔐 Verifying admin status server-side...');
      ...
      console.log('🔐 Server-side admin verification result:', isAdmin);
      return isAdmin;
    } catch (error) {
      console.error('Error in server-side admin verification:', error);
      return false;
    }
  };
  ...
  useEffect(() => {
    const checkAdminStatus = async () => {
      ...
      try {
        ...
        const isAdminResult = await verifyAdminStatus();
  ...
  // User management actions (with server-side verification)
  ...
  return {
    ...
    updateSystemPrompt,
    createSystemPrompt,
    verifyAdminStatus // Expose for additional security checks
  };
};
